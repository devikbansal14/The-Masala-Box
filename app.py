...
from config import BRAND, ABOUT, SHIPPING, PROMO_CODES, FLASK_CONFIG, DATABASE, WHATSAPP, EMAIL, RAZORPAY

# Try to import razorpay (optional - will work without it in test mode)
try:
    import razorpay
    razorpay_client = razorpay.Client(auth=(RAZORPAY['key_id'], RAZORPAY['key_secret']))
    RAZORPAY_ENABLED = True
except ImportError:
    razorpay_client = None
    RAZORPAY_ENABLED = False
    print("Warning: razorpay package not installed. Payment gateway disabled.")
...
# Legacy config for templates (backwards compatibility)
SITE_CONFIG = {
    "about_us": ABOUT,
    "shipping": SHIPPING
}
...
def init_db():
    ...
    # Cart table (supports both logged-in users and guests)
    cursor.execute('''
...
def seed_products():
    """Seed database with products from products.py."""
    conn = sqlite3.connect(DATABASE['path'])
    cursor = conn.cursor()
    ...
    for product in ALL_PRODUCTS:
        cursor.execute('''
            INSERT INTO products (name, subtitle, tagline, description, price, category,
                                image_url, rating, reviews, allergy_info)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            product['name'],
            product['subtitle'],
        ...
        for ingredient in product.get('ingredients', []):
            cursor.execute(
                'INSERT INTO ingredients (product_id, ingredient_name) VALUES (?, ?)',
...
def merge_guest_cart_to_user(user_id):
    """Merge guest cart items into user's cart after login/signup."""
    if 'session_id' not in session:
        return
    ...
    cursor.execute(
        'SELECT product_id, quantity, weight, price_override FROM cart WHERE session_id = ? AND user_id IS NULL',
        (session_id,)
    )
    guest_items = cursor.fetchall()

    for item in guest_items:
        product_id, quantity, weight, price_override = item
        ...
        if existing:
            ...
            cursor.execute(
                ...
                (user_id, product_id, quantity, weight, price_override)
            )

    # Delete guest cart items
    cursor.execute('DELETE FROM cart WHERE session_id = ? AND user_id IS NULL', (session_id,))
...
if __name__ == '__main__':
    ...
    print(f"Server running at http://localhost:{FLASK_CONFIG['port']}")
